...
Human scheduling of random dot motion tasks
...
Future experiments will additionaly vary the utility/reward of each subtask, the overall goal, and whether or not the difficulty needs to be learned or is described. 
...
RDK difficulty will be constant across trials and labelled as Easy, Medium, Hard, or Very Hard. RDKs will be presented in onscreen at the four compass points (North, South, East, West). Location of each difficulty level will be averaged across participants. 
### 4.	Hypotheses
...
1. The optimal policy to minimize the time on any task is to complete shortest subtask first. We will determine whether participants converge to this optimal schedule. 
...
Difficulty location will be randomized across subjects. Four coherence levels will be used to manipulate difficulty (proportions of 0.8, 0.5, 0.1, and 0.0). A single deadline of 6 seconds will be tested.
...
We will recruit 50 participants.

Each participant will complete 30 scheduling trials via a web browser.
...
We manipulated the coherence and direction of four RDK tasks and the deadline available for selection and completion of the four tasks on each trial.
...
The main variable is the order in which subtasks are collected.

Secondary variables are the accuracy and completion time of the RDK subtasks. 
...
Posterior parameter estimation will be used for inference.
