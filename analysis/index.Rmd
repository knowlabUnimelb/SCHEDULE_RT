...
`r fd$nSubjects` were assigned to the _Fixed Difficulty_ condition. In this condition, the location of easy, medium, hard, and very hard random dot kinematograms (RDK's) was held constant across trials. 

`r rd$nSubjects` were assigned to the _Random Difficulty_ condition. In this condition, the location of easy, medium, hard, and very hard random dot kinematograms (RDK's) were randomized from trial to trial. The Fixed Difficulty condition was completed before the Random Difficulty condition. Participants only completed one of these conditions.

## Design

In each condition, participants completed multiple trials in which they selected and completed RDK tasks. On each trial, participants were shown a set of four RDKs labelled Easy, Medium, Hard, and Very Hard. The labels corresponded to the difficulty of the RDK. The proportion of dots moving in a coherent direction was set to .8, .5, .2, and .0 for the Easy, Medium, Hard, and Very Hard locations, respectively.
...
difficulty = aggregate(list("accuracy" = rdk$correct, "rt" = rdk$rt), by = list("difficulty" = rdk$dot_coherence, "phase" = rdk$phase), FUN = function(x) mean(x, na.rm = TRUE))
...
On average, across both conditions, participants completed `r avgCompletions$x[avgCompletions$phase == "no deadline"]` tasks during the `r as.character(avgCompletions$phase[1])` phase and `r avgCompletions$x[avgCompletions$phase == "deadline"]` tasks during the `r as.character(avgCompletions$phase[2])` phase. 
...
* TODO: What is the proportion of easy-task-first choices?

# Optimality in each condition

The task set presented to each subject will have an optimal solution (ordered from easiest to most difficult). The first analysis will compute something like Kendall's Tau, rank-order distance, which ranges between 0 (perfect match) and 6 (maximally distant), for 4 options. 
...
get_trial_scores = function(sdata, order_to_compare, ntrials, n, nperms, allperms, nSubjects, subjects){
  ...
  return(list("max_distances" = max_distances, "min_distances" = min_distances, "avg_distances" = avg_distances))
}
...
max_plot <- avg_maxdata %>% ggplot(aes(x=avg, fill=phase)) + geom_histogram(color="white", alpha=0.6, bins = 10, position="identity") + facet_wrap(~condition+phase)

min_plot <- avg_mindata %>% ggplot(aes(x=avg, fill=phase)) + geom_histogram(color="white", alpha=0.6, bins = 10, position="identity") + facet_wrap(~condition+phase)

avg_plot <- avg_avgdata %>% ggplot(aes(x=avg, fill=phase)) + geom_histogram(color="white", alpha=0.6, bins = 10, position="identity") + facet_wrap(~condition+phase)
...
spatial_scores = list("fixed" = list("practice" = get_spatial_consistency(splitData(fd$data)$practice, spatial_orders, fp$ntrials, 
                                                         n, fd$nSubjects, fd$subjects), 
                            "experiment" = get_spatial_consistency(splitData(fd$data)$experiment, spatial_orders, fe$ntrials,
                                                         n, fd$nSubjects, fd$subjects)), 
             "random" = list("practice" = get_spatial_consistency(splitData(rd$data)$practice, spatial_orders, rp$ntrials, 
                                                         n, rd$nSubjects, rd$subjects), 
                            "experiment" = get_spatial_consistency(splitData(rd$data)$experiment, spatial_orders, re$ntrials,
                                                         n, rd$nSubjects, rd$subjects)))
...
spatial_choice_plot <- avg_scdata %>% ggplot(aes(x=avg, fill=phase)) + geom_histogram(color="white", alpha=0.6, bins = 10, position="identity") + facet_wrap(~condition+phase)
