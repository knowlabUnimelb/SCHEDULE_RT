---
title: "Experiment 4: Pixel Brightness with error delay"
...
# Notes: 
# odd problme with uniqueid 26323 so remove it
...
fixed_datafilename = "exp4_fixed_pix_data.csv"
...
fixd_rawdata = fixd_rawdata[fixd_rawdata$uniqueid != 26323, ]
...
fixd_rawdata$darkness = as.integer(fixd_rawdata$darkness)
fixd_rawdata$dot_difficulty = as.numeric(fixd_rawdata$dot_difficulty)
fixd_rawdata$response = as.integer(fixd_rawdata$response) # -1's are patches that were not completed, time outs
...
# Remove trials with rt recorded as NA
fixd_rawdata = fixd_rawdata[!is.na(fixd_rawdata$rt), ]
...
We tested `r fd$nSubjects` participants (`r fixedDemo$nFemales` F, `r fixedDemo$nMales` M, `r fixedDemo$nOthers` Undeclared). Participants were recruited through the Melbourne School of Psychological Sciences Research Experience Pool (Mean age = `r round(mean(fixedDemo$ages, na.rm=TRUE),2) `, range = `r min(fixedDemo$ages, na.rm=TRUE) ` - `r max(fixedDemo$ages, na.rm=TRUE)`) and  were reimbursed with credit toward completion of a first year psychology subject. Datasets from `r fd$nRepeats` subjects were excluded for completing the experiment twice; i.e., only the first of the datasets for these subjects was retained. One participant was removed prior to data analysis due to errors in the trial numbering in their datafile.


## Design

```{r getCoherence, echo=FALSE, warning=FALSE, message=FALSE, results="hide"}
coherence_set = sort(unique(c(data$patch_0, data$patch_1, data$patch_2, data$patch_3)), decreasing = FALSE)
...
# Note: data analyisis was written for rdk task so maintain that variable name but note that it refers to pixel task here

rdk = data[data$trial_event == "subtask", ]
...
rdk.easyacc = aggregate(rdk$correct[rdk$dot_difficulty == 1], by=list("condition" = rdk$condition[rdk$dot_difficulty == 1], "phase" = rdk$phase[rdk$dot_difficulty == 1], "subject" = rdk$uniqueid[rdk$dot_difficulty == 1]), FUN=mean)
...
`r nNonlearners` subjects had less than chance accuracy on the easiest RDK; we removed these participants from further anlaysis leaving `r nRemainingFixed`.
...
tc.aov = aov(x ~ phase, data = tcdata)
tc.ml0 = lm(x ~ phase, data = tcdata)
tc.ml1 = lmer(x ~ phase + (1|subject), data = tcdata)
...
rdk.sumrt = aggregate(list("sumrt" = rdk$rt), by = list("subject"=rdk$subject, "condition"=rdk$condition, "phase"=rdk$phase, "difficulty"=rdk$dot_difficulty, "trial"=rdk$trial_number), FUN = function(x) tail(cumsum(x), n=1)+((length(x)-1) * 500))
...
anova.sumrt = anova_test(data = aov.sumrt.data, dv = rt, wid = subject, within = c(phase, difficulty))
...
# Difficulty plot
rdk$dot_difficulty = as.factor(rdk$dot_difficulty)
levels(rdk$dot_difficulty) = c("easy", "medium", "hard", "very hard")

diff_plot <- rdk %>% ggplot(aes(x=dot_difficulty, y=rt, fill=dot_difficulty)) + geom_violin(adjust = 1.5, trim = TRUE) + facet_wrap(~phase) + labs(y="RT (msec)", x = "Difficulty")
...
levels(rdk.sumrt$difficulty) = c("easy", "medium", "hard", "very hard")
...
difficulty = aggregate(list("accuracy" = rdk$correct, "rt" = rdk$rt), by = list("difficulty" =        rdk$dot_difficulty, "phase" = rdk$phase), FUN = function(x) mean(x, na.rm = TRUE))

sum_difficulty = aggregate(list("sumrt" = rdk.sumrt$sumrt), by = list("difficulty" = rdk.sumrt$difficulty, "phase" = rdk.sumrt$phase), FUN = function(x) mean(x, na.rm = TRUE))
                
diffsd = aggregate(list("accuracy" = rdk$correct, "rt" = rdk$rt), by = list("difficulty" =  rdk$dot_difficulty, "phase" = rdk$phase), FUN = function(x) sd(x, na.rm = TRUE))

sum_diffsd = aggregate(list("sumrt" = rdk.sumrt$sumrt), by = list("difficulty" =  rdk.sumrt$difficulty, "phase" = rdk.sumrt$phase), FUN = function(x) sd(x, na.rm = TRUE))

ncounts = rdk %>% count(phase, dot_difficulty)
...
rrdata = aggregate(list("accuracy" = rdk$correct), by = list("difficulty" =        rdk$dot_difficulty, "phase" = rdk$phase, "subject"=rdk$subject), FUN = function(x) mean(x, na.rm = TRUE))
rrdata_sumrt = aggregate(list("sumrt" = rdk.sumrt$sumrt), by = list("difficulty" = rdk.sumrt$difficulty, "phase" = rdk.sumrt$phase,  "subject"=rdk.sumrt$subject), FUN = function(x) mean(x, na.rm = TRUE))
...
levels(difficulty$difficulty) = c("Easy", "Medium", "Hard", "Very Hard")
```


```{r difficulty_table, echo=FALSE, warning=FALSE, message=FALSE, results="asis"}
# Create the table
difficulty = difficulty[c("phase", "difficulty", "n", "accuracy", "se.acc", "rt", "se.rt", "sumrt", "se.sumrt")]
...
splitData = function(data){
  pracdata = data[data$trial_event == "practice_pix", ]
  exp_data = data[data$trial_event == "select_pix", ]
...
get_trial_scores = function(sdata, ntrials, n, nperms, allperms, nSubjects, subjects){
  ...
  for (j in 1:nSubjects){
    ...
    for (i in 1:ntrials){
        ...
        # If the subject did not select any task and the task times out, then there won't be an entry in the data file, so make sure there is         some
        if (nrow(tdata[tdata$trial_number == i-1, ]) > 0){
        ...
        if (i == 24 && j == 16){
          order_to_compare$x = c(NA, NA, NA, NA)
          order_to_compare$ix = c(2, 4, 3, 1)
          } else {
...
max_a = setNames(aggregate(expdata$max_practice, by=list( expdata$subject), mean), c("subject", "avg"))
max_b = setNames(aggregate(expdata$max_expFirst10, by=list(expdata$subject), mean), c( "subject", "avg"))
max_c = setNames(aggregate(expdata$max_expLast10, by=list( expdata$subject), mean), c( "subject", "avg"))
...
min_a = setNames(aggregate(expdata$min_practice, by=list( expdata$subject), mean), c( "subject", "avg"))
min_b = setNames(aggregate(expdata$min_expFirst10, by=list( expdata$subject), mean), c( "subject", "avg"))
min_c = setNames(aggregate(expdata$min_expLast10, by=list( expdata$subject), mean), c("subject", "avg"))
...
avg_a = setNames(aggregate(expdata$avg_practice, by=list( expdata$subject), mean), c("subject", "avg"))
avg_b = setNames(aggregate(expdata$avg_expFirst10, by=list( expdata$subject), mean), c("subject", "avg"))
avg_c = setNames(aggregate(expdata$avg_expLast10, by=list( expdata$subject), mean), c("subject", "avg"))
...
max_plot <- avg_maxdata %>% ggplot(aes(x=avg, fill=phase)) + geom_histogram(color="white", alpha=0.6, bins = 10, position="identity") + facet_wrap(~phase) + labs(y="Frequency", x = "Max Distance")

min_plot <- avg_mindata %>% ggplot(aes(x=avg, fill=phase)) + geom_histogram(color="white", alpha=0.6, bins = 10, position="identity") + facet_wrap(~phase) + labs(y="Frequency", x = "Min Distance")

avg_plot <- avg_avgdata %>% ggplot(aes(x=avg, fill=phase)) + geom_histogram(color="white", alpha=0.6, bins = 10, position="identity") + facet_wrap(~phase) + labs(y="Frequency", x = "Avg Distance")
...
ks.fixed.phase1 = ks.test(allrand$s, avg_avgdata$avg[avg_avgdata$phase == "untimed practice"])$statistic/ks.test(twofix$s, avg_avgdata$avg[avg_avgdata$phase == "untimed practice"])$statistic

ks.fixed.phase2 = ks.test(allrand$s, avg_avgdata$avg[avg_avgdata$phase == "deadline early"])$statistic/ks.test(twofix$s, avg_avgdata$avg[avg_avgdata$phase == "deadline early"])$statistic

ks.fixed.phase3 = ks.test(allrand$s, avg_avgdata$avg[avg_avgdata$phase == "deadline late"])$statistic/ks.test(twofix$s, avg_avgdata$avg[avg_avgdata$phase == "deadline late"])$statistic

# Build data for plotting
randomness = as.data.frame(list("phase" = factor(c("untimed", "deadline early", "deadline late", "untimed", "deadline early", "deadline late"), levels = c("untimed", "deadline early", "deadline late")), "ratio" = c(ks.fixed.phase1, ks.fixed.phase2, ks.fixed.phase3)))
levels(randomness$phase) = c("untimed", "deadline early", "deadline late")

# Plot
random_eval_plot <- randomness %>% ggplot(aes(x=phase, y=ratio)) + 
                     geom_point(size=4) + 
...
levels(avg_scdata$condition) = c("fixed") # Replace condition names
...
levels(avg_sddata$condition) = c("fixed") # Replace condition names

```
```{r plotSpatialData, echo=FALSE, warning=FALSE, message=FALSE, results="asis"}
spatial_choice_plot <- avg_scdata %>% ggplot(aes(x=avg, fill=phase)) + geom_histogram(color="white", alpha=0.6, bins = 10, position="identity") + facet_wrap(~phase) + labs(y="Frequency", x = "Proportion of Spatially Consistent Responses") + ggtitle("Spatial Strategy Analysis")
